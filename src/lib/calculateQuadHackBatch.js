 ...
 * @param hacksPerThread The number of hacks to do per thread. Should be 1 with the exception of quadhack single batch farm.
 ...
 */
export function calculateQuadHackBatch (ns, target, threads, hacksPerThread = 1, opsBuffer = DEFAULT_OPS_BUFFER, cycleBuffer = DEFAULT_CYCLE_BUFFER) {
  if (ns === undefined) {
  ...
  const cycleTime = Math.max(totalHackTime + (opsBuffer * 2) + cycleBuffer, targetInfo.weakenTime + cycleBuffer)
