...
/**
 * Farms enough to buy FTPCrack.exe
 *
 * @param {NS} ns NS
 */
export async function goalTwoAsync (ns) {
  if (ns === undefined) {
  ...
  if (ns.fileExists('FTPCrack.exe')) {
    ns.tprint('FTPCrack.exe detected, skipping goal!')
    return
  }

  if (ns.getServerMoneyAvailable(HOME) < 1500000) {
    ...
    ns.tprint('Waiting for you to have enough money to buy FTPCrack.exe')
    while (ns.getServerMoneyAvailable(HOME) < 1500000) {
      await ns.asleep(10)
    }
  ...
  ns.tprint('Waiting for you to buy FTPCrack.exe, you should have enough')
